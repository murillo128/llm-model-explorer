...
Scientific context uses a reusable compact title bar with identity, information, secondary metadata, reserved operation status, and a composable right-side action slot. The title bar and scientific body belong to one rounded card. The outer card owns the continuous `1 px` border, `9 px` corner radius, surface background and subtle base shadow. Its full-width title row is `40 px` high, with a soft-neutral background rounded at the top corners and one internal bottom separator, and remains stationary above scrolling data. The body contains the scientific view and its local inspection/error surfaces with `12 px` padding (`8 px` at the narrow breakpoint), without a second outer border or radius. Transient status must not insert content between the title and matrix or change their geometry. Information floats below its trigger; the rounded card must not clip it or reserve a permanent metadata column. Tensor-specific fields and preview/pinned dismissal behavior belong to `tensor-explorer.md`.
