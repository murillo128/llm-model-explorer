...
The graph binds only to inventory identities that actually exist and to documented storage relationships. Unresolved or unavailable numeric views retain their parameter descriptions and reasons, but never fabricate a usable tensor endpoint. The original checkpoint stays authoritative. New decoders, arbitrary region extraction, rank slicing, and raw-storage exploration are not required.
