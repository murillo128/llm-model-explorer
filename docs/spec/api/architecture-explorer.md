...
`ArchitectureProvenance` records use `kind: configuration | storage | description`, with a safe `source` identifier and optional `revision`/`rule`. Configuration sources may be JSON pointers; storage sources are tensor names; description sources identify packaged descriptions/reviewed rules. Provenance is evidence of origin, not proof that inference ran. `ArchitectureDiagnostic` requires `code` and `message`, with optional `node_id` or `parameter_id` targeting a record in this graph. Model-owned load failures encode the validation stage and `architecture.json` JSON pointer in a bounded safe message, since an unavailable result has no graph-local target. It must not expose traceback or filesystem content.

## Quantized checkpoint capability separation

The existing tensor endpoints remain logical-value endpoints, not a raw quantization inspector. Retain a complete validated physical inventory internally. Publish through `TensorInventory.tensors` complete logical tensors whose native or admitted packed decoder path and role/geometry are verified. Admitted GPTQ Int4, ModelOpt NVFP4, bitsandbytes NF4 with double quantization, and compressed-tensors `pack-quantized` W4A16 INT4 groups each expose one logical `…weight` matrix in `[output, input]` order, as defined by [backend model admission](../backend/models.md#admitted-packed-weight-decoding). Encoding auxiliaries remain internal. A selected LoRA composition also publishes its validated `lora_A` and `lora_B` factors as ordinary logical tensors alongside the base inventory; both remain bound to their original Safetensors and never replace or merge a base weight. The backend owns decoding; data, statistics, distributions and architecture inspection use the same logical tensor identity and float32 values.
...
For a selected PEFT composition, the ordinary graph nodes and parameter records describe the verified LoRA branch and refer to the composition's `lora_A`/`lora_B` tensor IDs. They use the existing graph schema and weight-inspection capability; no merged-weight ID or new execution endpoint is introduced.
...
The backend is the semantic authority for accepted graphs. Reject unknown discriminator values and malformed records rather than silently rendering approximate structure. Check ID uniqueness, reference closure, parent/child agreement, acyclic containment, repetition membership/order, alias termination, port directions, parameter inspection membership in the session inventory, and resolvable shape symbols. Check all integer products/ranges with the existing safe-integer rules. Disagreement of known connected dimensions must be diagnosed, not overwritten by the frontend. Browser clients validate the generated response schema and session identity; they do not reimplement graph relationship, template or binding validation.
