...
`inspection` is a discriminated object. With `status: available`, it contains a real existing inventory `tensor_id` for one complete, native, logical rank-1/rank-2 tensor. With `status: unavailable`, it contains `reason` (`unsupported_representation`, `unsupported_rank`, `requires_view`, or `unresolved_binding`) and a safe `message`, and no fabricated actionable `tensor_id`. A known native tensor of higher rank retains its storage/logical descriptor even though this modal cannot render it. Aliases may resolve to an existing native tensor only when identity and full geometry are verified. Packed integers, scales, arbitrary regions, and flattened higher-rank weights are not offered as substitute logical matrices.
...
The existing tensor endpoints remain logical-value endpoints, not a raw quantization inspector. For newly admitted quantized checkpoints, retain a complete validated physical inventory internally. Publish through `TensorInventory.tensors` only complete logical tensors whose native data path is already supported and whose role/geometry has been verified; packed values, scales, and unresolved regions remain parameter/storage descriptions in the graph rather than pretend float32 tensors.

Extend `TensorInventory` with `coverage: complete | partial` and `diagnostics` (safe code/message records without graph-local targets). Unquantized baseline inventories retain every prior tensor and report complete. Quantized inventories report partial when entries are deliberately not exposed through numeric endpoints, including when no native logical tensors are currently available. This is an explicit limitation, not a silent complete-inventory claim. Do not change existing tensor IDs or materialized bytes. Unknown/non-exposed IDs retain the existing `tensor_not_found` response; known but unsupported operations retain their existing typed errors.

Tokenizer availability continues to be independent of tensor and architecture coverage. A V-JEPA 2 session is valid without text-tokenizer assets; clients must not issue a text-tokenization request merely to retrieve its graph. Input-embedding lookup must not fall back to a packed tensor or a V-JEPA visual representation. New quantized mathematical decoders are outside this contract.
