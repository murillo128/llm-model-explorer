...
`GET /models` exposes selectable models discovered below the backend-configured local model root plus a `diagnostics` array for recognized local PEFT adapter candidates rejected during validation or composition. Rejected adapters are not model descriptors and cannot be selected for sessions. Each diagnostic has a safe candidate label, code, message, and an optional path-free base model identity; it never exposes a filesystem path. The array is empty when no adapter candidate was rejected. A model descriptor may include architecture, model type, parameter count, total local size, and tokenizer availability when those values can be derived reliably. Local paths and the internal content fingerprint are not exposed.
