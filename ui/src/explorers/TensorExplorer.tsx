...
function LoadedTensorExplorer({ client, sessionId, selectedTensor, selection, showInformation }: ExplorerContextValue & { showInformation?: boolean }) {
  ...
  return <section className="tensor-explorer" aria-label="Tensor scientific view">
    <MatrixExplorer source={source} label="Tensor matrix; scroll to inspect all values"
      header={(cameraControls, domain) => <TensorHeader tensor={selectedTensor!} domain={domain} showInformation={showInformation} status={!allocationFailed && <div className="tensor-results" aria-label="Result status">
        ...
        aria-label="Cancel loading" title="Cancel loading" onClick={() => controller.current?.cancel()}>×</button>}{cameraControls}</>} />}
