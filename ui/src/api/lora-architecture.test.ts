...
it('validates and exposes native logical LoRA factor bindings in the generic graph', () => {
  ...
  const context = { modelId: 'lab/alpha', inventory, tokenizerAvailable: true };
  ...
  expect(() => validateArchitecture(invalid, context)).toThrow('Native adapter factor storage identity/geometry');
});
