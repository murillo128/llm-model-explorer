  ...
  it('does not splice an external re-entry or bypass into an internal path', () => {
    ...
    const graph: Graph = { graph_id: 'reentrant', scope: 'language_model', coverage: 'partial', symbols: [], parameters: [], repetitions: [], diagnostics: [],
      nodes: [{ ...base, id: 'component', kind: 'group', label: 'Component', children: ['a', 'b'] },
        { ...base, id: 'outside', kind: 'group', label: 'External boundary', children: [] },
        ...['a', 'b'].map((id) => ({ ...base, id, kind: 'operation' as const, label: id, parent_id: 'component' }))],
      edges: [
        { id: 'exit', source: ep('a', 'out'), target: ep('outside', 'in'), kind: 'state', provenance: [] },
        { id: 'reenter', source: ep('outside', 'in'), target: ep('b', 'in'), kind: 'state', provenance: [] },
        { id: 'bypass', source: ep('outside', 'in'), target: ep('outside', 'out'), kind: 'data', provenance: [] },
      ] };
    const projection = sourcePaths(graph, 'component');
    expect(projection.edges.flatMap((edge) => edge.paths.map((path) => path.map((item) => item.id)))).toEqual([['exit'], ['reenter']]);
    expect(projection.scope!.excludedEdgeIds).toEqual(['bypass']);
