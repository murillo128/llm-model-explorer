...
export interface ProjectedNode {
  ...
  presentation?: 'repetition' | 'range' | 'mlp'; repetitionId?: string;
...
export function projectGraph(graph: Graph, options: ProjectionOptions): Projection {
  ...
  for (const n of graph.nodes) if (!n.parent_id) visit(n.id);
  ...
  function emit(path: SourceEdge[]) {
    ...
    const sourceOwner = owners.get(source.node_id)!, targetOwner = owners.get(target.node_id)!;
    if (sourceOwner === targetOwner && (!sourceOwner.expanded || source.node_id !== target.node_id)) {
  ...
  for (const edge of graph.edges) {
    ...
    if (sOwner === tOwner && !sOwner.expanded) { hidden.add(edge.id); continue; }
